...
#
# -----------------------------------------------------------
def getApiResults(case, pod, scenario, version):
    """
    ...
    url = ("http://" + url_base + "?case=" + case +
           "&period=" + str(period) + "&pod_name=" + pod +
...
def getScenarios(project, case, pod, version, **kwargs):
    """
...
def getScenarioStatus(pod, version):
    """
...
def getCaseScore(testCase, pod, scenario, version):
    """
    Get Result  for a given Functest Testcase
    """
    # retrieve raw results
    results = getApiResults(testCase, pod, scenario, version)
...
def export_csv(scenario_file_name, pod, version):
    ...
    with open(scenario_file_name, "r") as scenario_file:
        ...
        for line in scenario_file:
            if pod in line:
                scenario_pod_file.write(line)
    scenario_pod_file.close
